              ...
              <div key={item.pergunta}>
                <h3>
                  <button
                    ...
                    className="flex w-full items-start justify-between gap-3 py-3.5 text-left text-[0.95rem] font-semibold text-cream-50 md:gap-4 md:py-5 md:text-xl"
                  >
                    {item.pergunta}
                    <span
                      aria-hidden="true"
                ...
                <div id={panelId} role="region" aria-labelledby={buttonId} hidden={!isOpen}>
                  <p className="pb-4 text-[0.85rem] leading-relaxed text-cream-200 md:pb-6 md:text-lg">{item.resposta}</p>
                </div>
              </div>
            )
          })}
